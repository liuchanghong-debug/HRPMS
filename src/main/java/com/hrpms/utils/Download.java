...
public class Download {
     ...
     * @return
     **/
    public static void contractUrlPreview(String name, HttpServletRequest request, HttpServletResponse response) throws Exception {
        File file = new File(request.getRealPath("contract\\" + name));
        BufferedInputStream bufferedInputStream = new BufferedInputStream(new FileInputStream(file));
        response.setHeader("Content-Disposition", "filename=" + DownLoadMessyCode.setDownloadFileName(name.substring(name.indexOf("_") + 1), request));
        ServletOutputStream outputStream = response.getOutputStream();
        IOUtils.copy(bufferedInputStream, outputStream);

package com.hrpms.service.business_menu_service.gongjijin_manager_service.impl;
...
public class TbAccumulationFundServiceImpl implements TbAccumulationFundService {
    ...
    public void AccumulationDownload(Map map, HttpServletResponse response, HttpServletRequest request) throws Exception {
        ...
        for(TbAccumulationFund accumulation:tbAccumulationFunds){
            ...
            if(accumulation.getStatus().equals("0")){
                datelist.add("未缴");
            }else{
                datelist.add("已缴");
            }
    ...
    public TbAccumulationFund selectAccumulationByName(String name) {
        ...
        return tbAccumulationFundDao.selectAccumulationByName(name);
    }

    @Override
    public List gongJiJinByStatusForPhone() {
        List<TbAccumulationFund> tbAccumulationFunds = tbAccumulationFundDao.gongJiJinByStatusForPhone();
        List list = new ArrayList();
        for(TbAccumulationFund fund:tbAccumulationFunds){
            TbCustomer customer = customerService.getCustomerByIdCard(fund.getIdCard());
            Map map = new HashMap();

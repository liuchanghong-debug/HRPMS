package com.hrpms.dao.system_setting_dao.role_manager_dao.impl;
...
public class TbSystemRoleDaoImpl implements TbSystemRoleDao {
    ...
    public List<TbSystemRole> selectAllRoleName() {
        Session session = sessionFactory.getCurrentSession();
        List<TbSystemRole> tbSystemRoles = session.createQuery("from TbSystemRole where status=0").list();
    ...
    public TbSystemRole roleNameIsOne(String roleName) {
        Session session = sessionFactory.getCurrentSession();
        TbSystemRole tbSystemRole = (TbSystemRole)session.createQuery("from TbSystemRole where roleName =? ").setParameter(0, roleName).uniqueResult();
        return tbSystemRole;
    }
}

...
<html style="overflow-x:auto;overflow-y:auto;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
        ...
        function callbackdata() {
            ...
            var subject = $("input[type='radio']:checked ").parent("td").next("td").next("td").html();
            var content = $("input[type='radio']:checked ").parent("td").next("td").next("td").next("td").html();
            var list = new Array();
            list[0]=id;
            list[1]=subject;
            list[2]=content;
...
<form id="searchForm" class="breadcrumb form-search" action="/marketing-manager/selectModleJsp" method="post">
	<ul class="ul-form">
		<li><label>标题：</label>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<c:forEach items="${page.dataList}" var="email">
		<tr>
			<td>
				<input type="radio" name="selectModle"/>
			</td>
			<td>
					${email.id}
			</td>
...
<div class="pagination"><ul>
	<li class="disabled"><a href="marketing-manager/selectModleJsp?currentPage=${page.currentPage-1}&subject=${map.subject}">上一页</a></li>
	<c:forEach begin="1" end="${page.pageCount}" var="num">
		<li class="active"><a href="marketing-manager/selectModleJsp?currentPage=${num}&subject=${map.subject}">${num}</a></li>
	</c:forEach>

	<li class="disabled"><a href="marketing-manager/selectModleJsp?currentPage=${page.currentPage+1}&subject=${map.subject}">下一页</a></li>

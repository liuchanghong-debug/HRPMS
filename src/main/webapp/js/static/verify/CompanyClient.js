...
$(function () {
    ...
    $("#status").change(function () {
        if($("#status").val() != ""  && $("#status").val() != null){
            $("#status ~ span").remove();
            $("#status").after(trueStyle);
            status = true;
        }else {
            $("#status ~ span").remove();
            $("#status").after(falseStyle);
        }
    });
    //公司类别
    $("#companyType").change(function () {
        if($("#companyType").val() != ""  && $("#companyType").val() != null){
            $("#companyType ~ span").remove();
            $("#companyType").after(trueStyle);
            companyType = true;
        }else {
            $("#companyType ~ span").remove();
            $("#companyType").after(falseStyle);
        }
    });
    //公司地址
...
function inputFormSubmit() {
    ...
    $("textarea").keyup();
    $("select").change();
    if(!(name && companyNo && telPhone && zipCode && owner && ownerShip && idCard && phone && email && status && companyType && address)){
